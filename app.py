    ...
    with st.form("form_kuesioner"):
        ...
        if submitted:
            if None in responses:
                ...
                st.session_state.corsi_level = 1     # Reset Level
                st.session_state.corsi_lives = 2     # Reset Nyawa Awal (2x Kesempatan)
                st.session_state.corsi_score = 0     # Reset Skor
                st.rerun()
...
# ==========================================
    ...
    if st.session_state.corsi_phase == "idle":
        with layar.container():
            ...
            # Logic Urutan: Level 1 = 2 blink, Level 9 = 10 blink
    ...
    # FASE 3: INPUT
        ...
        if len(st.session_state.corsi_user_input) > 0:
            ...
            if st.session_state.corsi_user_input[curr] != st.session_state.corsi_sequence[curr]:
                time.sleep(0.3) # Silent fail (jeda dikit)
                
                # Kurangi Nyawa
                st.session_state.corsi_lives -= 1
                
                if st.session_state.corsi_lives > 0:
                    # Masih punya kesempatan -> Ulangi Level yang sama
                    # (Sequence baru di level yg sama akan digenerate saat masuk 'idle')
                    st.session_state.corsi_phase = "idle" 
                    st.rerun()
                else:
                    # Nyawa habis (2x salah di level ini) -> STOP
            ...
            # 2. JIKA BENAR
                ...
                # Update Skor (Level Tertinggi yang diselesaikan)
                ...
                if st.session_state.corsi_level >= 9:
                    ...
                    st.session_state.corsi_lives = 2 # Reset nyawa jadi 2 untuk level baru
                    st.session_state.corsi_phase = "idle"
                st.rerun()
